...
class MusicDiscWidget:
    ...
    def __init__(self, game, disc_path, button_path, size=220, anchor="topright", margin=16, speed_deg=60):
        ...
        # Charge et rédimensionne le disque
        disc = pg.image.load(disc_path)
        disc = disc.convert_alpha() if disc.get_alpha() is not None else disc.convert()
        self.disc_base = pg.transform.smoothscale(disc, (size, size))

        # Charge et rédimensionne le bouton
        ...
        self.btn_surf = pg.transform.smoothscale(btn, (btn_size, btn_size))
